...
class Graph:
    ...
    def add_edge(self, head: str, tail: str, type: str, lable: Optional[str] = None) -> None:
        self.dot.edge(
            self.validate_name(head),
            self.validate_name(tail),
            lable,
            **apply_edge_style(type),
        )
    ...
    def render(self, format: str, cleanup: bool = True) -> None:
            ...
            for service in self.compose.services:
                ...
                for port in service.ports:
                    ...
                    self.add_edge(
                        ...
                        service.name,
                        "links",
                        lable=port.container_port + (("/" + port.protocol.value) if port.protocol != Protocol.any else "") + (("\n(" + port.app_protocol.value + ")") if port.app_protocol != AppProtocol.na else ""),
                    )
        ...
        if shutil.which("dot") is None:
            raise RuntimeError("Graphviz is not installed or not in your PATH.")

        print("Rendering graph to file...")
